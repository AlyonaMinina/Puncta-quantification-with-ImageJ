...
print(" ");
print("Welcome to the IJM for autophagic bodies density quantification in two fluorescent channels!");
print(" ");
...
ROI_height = 20;
ROI_width = 10;
prominence_for_Ch_1 = 40;
prominence_for_Ch_2 = 50;
Gaussian_Ch_1 = 0.07;
Gaussian_Ch_2 = 0.07;

Dialog.create("Please provide ROIs parameters for your images");
Dialog.addString("Channel 1 name:", Ch_1);
Dialog.addToSameRow();
Dialog.addString("Channel 2 name:", Ch_2);
Dialog.addNumber("Prominence value for Ch1:", prominence_for_Ch_1);
Dialog.addToSameRow();
Dialog.addNumber("Prominence value for Ch2:", prominence_for_Ch_2);
Dialog.addNumber("Gaussian blur value for Ch1:", Gaussian_Ch_1);
Dialog.addToSameRow();
Dialog.addNumber("Gaussian blur value for Ch2:", Gaussian_Ch_2);
...
Ch_2 = Dialog.getString();
prominence_for_Ch_1 = Dialog.getNumber();
prominence_for_Ch_2 = Dialog.getNumber();
Gaussian_Ch_1 = Dialog.getNumber();
Gaussian_Ch_2 = Dialog.getNumber();
number_of_ROIs = Dialog.getNumber();
ROI_height = Dialog.getNumber();
ROI_width = Dialog.getNumber();	
print(" The analysis is performed using \n Prominence value for Ch1 = " + prominence_for_Ch_1 + " and for Ch2 = " + prominence_for_Ch_2 + "\n Gaussian blur value for Ch1 = " + Gaussian_Ch_1 + " and for Ch2 = " + Gaussian_Ch_2);	
print(" ");	
print(" ");
...
for (i = 0; i < image_list.length; i++){
	...
	for ( r=0; r<ROI_number; r++ ) {
		...
		rename("Segmentation");
		run("8-bit");
		run("Gaussian Blur...", "sigma=" + Gaussian_Ch_1 + " scaled");
		run("Enhance Contrast...", "saturated=0.35");
		run("Find Maxima...", "prominence="+ prominence_for_Ch_1 +" exclude output=Count");
		Ch1_puncta = getResult("Count",  0);
		Column_1 = "Number of " + Ch_1 + " puncta in the ROI";
		Table.set(Column_1, current_last_row, Ch1_puncta, "Image Results");
		Column_2 = "Number of "+ Ch_1 + " puncta per 10 um2";
		Table.set(Column_2, current_last_row, 10*Ch1_puncta/area, "Image Results");
		run("Clear Results");
		//create a segmented image
		run("Find Maxima...", "prominence="+ prominence_for_Ch_1 +" exclude output=[Point Selection]");
		run("Flatten");
		run("RGB Color");
		...
		//create a directory for segmentaton previews
		...
		if (isOpen("Segmentation")){
			...
			rename("Segmentation");
			run("8-bit");
			run("Gaussian Blur...", "sigma=" + Gaussian_Ch_2 + " scaled");
			run("Enhance Contrast...", "saturated=0.35");
			run("Find Maxima...", "prominence="+ prominence_for_Ch_2 +" exclude output=Count");
			Ch2_puncta = getResult("Count",  0);
			Column_3 =  "Number of " + Ch_2 + " puncta in the ROI";
			Table.set(Column_3, current_last_row, Ch2_puncta, "Image Results");
			Column_4 = "Number of "+ Ch_2 + " puncta per 10 um2";
			Table.set(Column_4, current_last_row, 10*Ch2_puncta/area, "Image Results");			
			run("Clear Results");
			//create a segmented image
			run("Find Maxima...", "prominence="+ prominence_for_Ch_2 +" exclude output=[Point Selection]");
			run("Flatten");
			run("RGB Color");
			selectWindow("Micrograph");
			run("Magenta");
			...
			Column_5 = Ch_1 + " to " + Ch_2 + " puncta ratio";
			if (Ch2_Int==0) {
			...
			if (Ch1_Int/Ch2_Int > 1) {
				print("Warning! " + Ch_1 + " to " + Ch_2 + " ratio > 1 for ROI " + r+1); //warning specific for our OSA experiment
				print(" ");
			}
			
			Column_6 = Ch_2 + " to " + Ch_1 + " puncta ratio";
			if (Ch1_Int==0 ) {
...
Table.save(output_dir + "Puncta density IJM results for experiment " + original_folder_name + ".csv");
